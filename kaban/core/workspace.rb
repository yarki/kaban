module Workspace
	...
	def Workspace.save_raw(docs: docs, collection: collection)
	...
	def Workspace.load_raw(collection: collection)
		raw_file = "#{configatron.workspace.staging}/#{collection.name}.fetched.json"
		puts "load raw from #{raw_file}"
		JSON.parse(File.read(raw_file))
	end

	def Workspace.save_mapped(docs: docs, collection: collection)
	...
	def Workspace.load_mapped(collection: collection)
		mapped_file = "#{configatron.workspace.staging}/#{collection.name}.mapped.json"
